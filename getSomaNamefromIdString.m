function [ data ] = getSomaNamefromIdString( idString,varargin)
%% Read settings file.
...
neuron_idString = [response.swcTracings(:).neuron];
...
query = sprintf('{ tracings(queryInput: {swcTracingIds: "%s"}) { tracings { firstNode { brainArea {structureId atlasId safeName acronym structureIdPath } } } } }',selected(axonInd(1)).id);
[ response ] = callgraphql( settings.Database.TracingsUrl, query);



data = response.tracings.tracings.firstNode.brainArea
